...
let tr_type = function
  ...
  | Integer Int -> St_int
  | Void -> St_void
  | Array_type(Integer Int) -> St_intarray(42) (*todo, insertsize?*) 
...
let tr_variable (v: variable) =
  { s_var_name     = fst v.var_name ;
    ...
    s_var_arraysize = 0 ; (*non array, change for array*)
...
let tr_binary_op = function
  ...
  | Array_access -> failwith "tr_binary_op:should not happen" (*non_supported "array access"*)
...
let tr_formal_parameter (p: formal_parameter) =
  ...
  { s_var_name     = fst p.param_name ;
    ...
    s_var_arraysize = 0 ; (*non array, todo*)
    s_var_uniqueId = new_id () ; }

let is_var (e:expression) = match e with
  | Variable(n) -> Some(n)
  | _ -> None
...
let rec tr_expression (env: env) (e: expression) =
  ...
  | Binary(Array_access,a,e) -> (*only a[i] for the moment*)
     let va = is_var (fst a) and ve = is_var (fst e) in
     begin
       match va,ve with
       | Some(vaa),Some(vii) -> Se_arrayaccess(tr_variable_name env vaa,tr_variable_name env vii)
       | _ -> non_supported "a[e]"
     end
  | Binary (b, e0, e1) ->
...
and tr_initializer_opt (env: env) = function
  ...
  | Some (Array_init vel) ->
     begin
       let resul =
	 List.map ( fun varinit -> (*only expressions in array initialisation*)
		    match varinit with
		    | Expr_init(e) -> tr_expr_e env e
		    | _ -> non_supported "recursive array initialisation"
		  ) vel
       in Some (resul)
     end
...
let rec tr_statement_e (env: env) ((s, ext): statement_e): s_block * env =
  ...
  | Block_statement b -> (List.rev (tr_block env b)), env
...
let tr_java_prog (jp: java_prog): s_program*env =
