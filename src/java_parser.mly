...
| modifiers_opt var_type variable_declarators SEMICOLON 
    { List.rev_map
	(fun (n,i) -> let (vn,is_array) = n in
                      { var_modifiers   = fst $1;
                        var_type        = if is_array then Array_type(fst $2) else
					    (fst $2);
                        var_name        = vn;
                        var_initializer = i } ) $3 }
...
| IDENTIFIER                            { ($1,false) }
| variable_declarator_id LBRACK RBRACK  { (fst $1,true) }
...
| var_type variable_declarators
    { List.rev_map
        (fun (n,i) -> let (vn,b)=n in
          { var_modifiers   = [];
            var_type        = if b then Array_type(fst $1) else (fst $1);
            var_name        = vn;
            var_initializer = i },
          (snd vn) ) $2 }
| FINAL var_type variable_declarators
    { List.rev_map
        (fun (n,i) -> let (vn,b)=n in (*if b then array*)
          { var_modifiers   = [Final];
            var_type        =  if b then Array_type(fst $2) else (fst $2);
            var_name        = vn;
            var_initializer = i },
          (snd vn) ) $3 }
